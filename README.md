...
#    自动完成：连接 Tailscale（若安装）-> 拉起 DSH -> 启动网关 -> 打印访问地址

# 3. 访问
#    浏览器打开 http://<本机IP>:8080/dashboard ，输入 gateway\.env 中的 GATEWAY_PASSWORD
```
...
start-everything.cmd/.ps1      # 一键启动（Tailscale + DSH + 网关）
gateway/
├─ app/
...
- `PLAN.md` — 方案设计
- 更多工程档案（进展/技术链路/经验）见本地 `归档\` 目录（不随仓库分发）
